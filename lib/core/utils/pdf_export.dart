...
/// Generates multi-page downloadable PDFs with cover pages, executive
/// summaries, charts, team breakdowns, and actionable recommendations.
class PdfExport {
  // ─── Brand Palette (from design spec) ───
  static const _primary = PdfColor.fromInt(0xFF0A2C6B); // Deep Blue
  static const _gold = PdfColor.fromInt(0xFFD4AF37); // Gold accent
  static const _green = PdfColor.fromInt(0xFF28A745); // Positive
  static const _red = PdfColor.fromInt(0xFFFF6B6B); // Warning / attention
  ...
  /// Generate and download a comprehensive Engagement Report PDF.
  ///
  /// [users] - list of user maps with keys: name, role, org, status, kudos
  /// [organizationName] - name shown on cover page
  /// [dailyActiveUsers], [monthlyActiveUsers] - headline KPIs
  /// [totalKudosSent], [avgKudosPerUser] - kudos metrics
  /// [engagementRate] - percentage of active users
  /// [dailyChangePercent], [monthlyChangePercent] - trend deltas
  /// [engagementDays] - 9-element list of daily active counts for chart
  /// [engagementMonths] - 9-element list of monthly active counts for chart
  /// [engagementLabels] - 9-element list of date labels for x-axis
  /// [kudosSentByMonth], [kudosReceivedByMonth] - 5-element monthly chart
  /// [kudosMonthLabels] - 5-element month labels
  /// [managerKudos], [peerKudos], [familyKudos] - kudos by source
  /// [teamBreakdowns] - list of maps: teamName, staffCount, participation,
  ///     avgKudosPerStaff, topRecognizer, topValue, warnings (optional list)
  static Future<void> exportEngagementReport({
      ...
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.SizedBox(height: 12),
            pw.Row(children: [
              _kpiBox('Engagement Rate', '${engagementRate.toStringAsFixed(0)}%',
                  'Active / total users',
            ...
            pw.SizedBox(height: 20),
            // Attention areas
            if (users.where((u) => u['status'] == 'Pending').length > 0) ...[
              _sectionLabel('ATTENTION AREAS'),
              pw.SizedBox(height: 8),
              _bulletPoint(
                  '${users.where((u) => u['status'] == 'Pending').length} staff with pending compliance status'),
            ],
            pw.SizedBox(height: 16),
            _sectionLabel('RECOMMENDATIONS'),
            pw.SizedBox(height: 8),
            _bulletPoint('Schedule weekend recognition prompts to boost engagement'),
            _bulletPoint('Manager check-ins with inactive staff members'),
    ...
    // ── PAGE 2: DAILY VS MONTHLY ACTIVE USERS ──
        ...
        pw.Page(
          ...
          build: (ctx) => pw.Column(
            ...
            children: [
              ...
              pw.SizedBox(height: 20),
              _buildBarChart(
                data1: engagementDays,
                data2: engagementMonths,
              ...
              _statRow('Peak DAU',
                  '${engagementDays.isNotEmpty ? engagementDays.reduce(max).toInt() : 0}'),
              _statRow('Average DAU',
                  '${engagementDays.isNotEmpty ? (engagementDays.reduce((a, b) => a + b) / engagementDays.length).toStringAsFixed(0) : 0}'),
              _statRow('MAU Trend',
                  '${_fmtPct(monthlyChangePercent)} over last month'),
              _statRow('Stickiness (DAU/MAU)',
                  '${stickiness.toStringAsFixed(0)}%'),
              pw.SizedBox(height: 20),
              _insightBox(
                stickiness >= 35
                    ? 'Your stickiness ratio of ${stickiness.toStringAsFixed(0)}% indicates healthy, recurring engagement. Top quartile care homes achieve 40%+ through consistent recognition habits.'
                    : 'Your stickiness ratio of ${stickiness.toStringAsFixed(0)}% has room for improvement. Encourage daily recognition habits to build towards the 40%+ target.',
              ),
              pw.Spacer(),
              _buildPageFooter(ctx),
            ],
    ...
    // ── PAGE 3: KUDOS SENT VS RECEIVED ──
    final totalKudos = managerKudos + peerKudos + familyKudos;
    if (kudosSentByMonth.isNotEmpty || totalKudos > 0) {
      pdf.addPage(
        pw.Page(
          pageFormat: PdfPageFormat.a4,
          margin: const pw.EdgeInsets.all(40),
          build: (ctx) => pw.Column(
            ...
            children: [
              ...
              if (kudosSentByMonth.isNotEmpty)
                _buildBarChart(
                  data1: kudosSentByMonth,
                  data2: kudosReceivedByMonth,
                  labels: kudosMonthLabels,
                  label1: 'Sent',
                  label2: 'Received',
                  color1: _primary,
                  color2: _green,
              ...
              pw.SizedBox(height: 10),
              _statRow('Total Kudos Sent', _fmtNum(totalKudosSent)),
              pw.SizedBox(height: 8),
              pw.Text('By Role:',
                  style: pw.TextStyle(
                      fontSize: 11, fontWeight: pw.FontWeight.bold, color: _darkText)),
              pw.SizedBox(height: 4),
              if (totalKudos > 0) ...[
                _bulletPoint(
                    'Managers gave: $managerKudos (${(managerKudos / totalKudos * 100).toStringAsFixed(0)}%)'),
                _bulletPoint(
                    'Peers gave: $peerKudos (${(peerKudos / totalKudos * 100).toStringAsFixed(0)}%)'),
                _bulletPoint(
                    'Families gave: $familyKudos (${(familyKudos / totalKudos * 100).toStringAsFixed(0)}%)'),
              ],
              pw.SizedBox(height: 16),
              _insightBox(
                peerKudos > managerKudos
                    ? 'Peer recognition dominates - this indicates a healthy recognition culture where colleagues actively celebrate each other.'
                    : 'Manager recognition leads - encourage peer-to-peer recognition to build a more distributed culture of appreciation.',
              ),
              pw.Spacer(),
              _buildPageFooter(ctx),
            ],
        ...
        pw.Page(
          ...
          build: (ctx) => pw.Column(
            ...
            children: [
              ...
              pw.SizedBox(height: 16),
              ...teamBreakdowns.map((team) => pw.Padding(
              ...
              pw.SizedBox(height: 8),
              _bulletPoint(
                  'Targeted engagement boost for teams below 60% participation'),
              _bulletPoint(
                  'Manager recognition focus for underperforming wings'),
              _bulletPoint(
                  'Identify and support inactive staff through check-ins'),
              pw.Spacer(),
              _buildPageFooter(ctx),
            ],
          ),
          ...
          build: (ctx) => [
            ...
            pw.TableHelper.fromTextArray(
              headerStyle: pw.TextStyle(
                  fontWeight: pw.FontWeight.bold, fontSize: 9, color: _white),
    ...
    await _download(pdf, 'CareKudos_Engagement_Report_${_ts(now)}.pdf');
  }
  ...
  /// Generate and download a comprehensive Training & Compliance Report PDF.
  ///
  /// [users] - user maps: name, role, org, gdpr, onboarding, status
  /// [gdprPercent], [onboardingPercent] - headline completion rates
  /// [organizationName] - cover page org name
  /// [totalUsers] - total staff count
  /// [gdprCompletedCount], [onboardingCompletedCount] - absolute numbers
  /// [nonCompliantCount] - staff missing GDPR training
  /// [teamComplianceBreakdown] - list of maps: teamName, percent
  /// [staffNeedingTraining] - list of maps: name, team, daysActive
  /// [cqcEvidencePosts] - list of maps: quote, value, stars
  /// [valuesDistribution] - list of maps: value, count, percent
  static Future<void> exportComplianceReport({
    ...
    final gdprDone = gdprCompletedCount > 0
        ? gdprCompletedCount
        : (userCount * gdprPercent / 100).round();
    final onbDone = onboardingCompletedCount > 0
        ? onboardingCompletedCount
        : (userCount * onboardingPercent / 100).round();
    final nonCompliant =
        nonCompliantCount > 0 ? nonCompliantCount : userCount - gdprDone;
      ...
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.Row(children: [
              _complianceGauge('GDPR TRAINING\nCOMPLETION', gdprPercent,
                  '$gdprDone of $userCount users'),
              pw.SizedBox(width: 16),
              _complianceGauge('ONBOARDING\nCOMPLETION', onboardingPercent,
                  '$onbDone of $userCount users'),
            ]),
            pw.SizedBox(height: 16),
            if (nonCompliant > 0)
              _alertBox(
                  'CRITICAL: $nonCompliant staff missing GDPR training. GDPR training is mandatory for CQC compliance.'),
            pw.SizedBox(height: 20),
            _sectionLabel('TRENDS'),
            pw.SizedBox(height: 8),
            _bulletPoint(
                'Training completion: ${gdprPercent.toStringAsFixed(0)}% of staff'),
            _bulletPoint(
                'Onboarding completion: ${onboardingPercent.toStringAsFixed(0)}% of staff'),
            _bulletPoint('Total staff requiring action: $nonCompliant'),
            pw.SizedBox(height: 20),
            _sectionLabel('CQC EVIDENCE READY'),
            pw.SizedBox(height: 8),
            pw.Text(
              'The following evidence is available for inspection:',
            ...
            _bulletPoint(
                'Peer recognition patterns (${users.length} user records)'),
            pw.Spacer(),
            _buildPageFooter(ctx),
          ],
        ),
    ...
    // ── PAGE 2: TRAINING COMPLETION DETAIL ──
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(40),
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            if (teamComplianceBreakdown.isNotEmpty) ...[
              ...
              pw.SizedBox(height: 8),
              _horizontalBar(
                  label: 'Onboarding', percent: onboardingPercent),
              pw.SizedBox(height: 16),
            ],
            // Staff needing training table
            _sectionLabel('Staff Needing Training'),
            pw.SizedBox(height: 10),
            if (staffNeedingTraining.isNotEmpty)
              pw.TableHelper.fromTextArray(
                headerStyle: pw.TextStyle(
                ...
                headerPadding:
                    const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 7),
                headers: ['Staff Name', 'Team', 'Days Active'],
                data: staffNeedingTraining
                    .map((s) => [
                          s['name'] ?? '',
                          s['team'] ?? '',
                          s['daysActive'] ?? '',
              ...
              _pendingTrainingTable(users),
            pw.SizedBox(height: 20),
            _sectionLabel('RECOMMENDATIONS'),
            pw.SizedBox(height: 8),
            _bulletPoint(
                'Schedule mandatory training sessions for non-compliant staff'),
            _bulletPoint(
                'Use tablet in staff room for quick completion during breaks'),
            _bulletPoint(
                '5-minute daily training slots during handover periods'),
            _bulletPoint(
                'Manager accountability: set target completion dates'),
            pw.Spacer(),
            _buildPageFooter(ctx),
          ],
        ),
      ...
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.SizedBox(height: 10),
            pw.Text('Staff Recognition Examples:',
                style: pw.TextStyle(fontSize: 10, color: _gray)),
            pw.SizedBox(height: 8),
            if (cqcEvidencePosts.isNotEmpty)
              ...
              _quoteBox(
                quote: 'Recognition data from your CareKudos platform will appear here once posts are captured.',
                value: 'Values',
                stars: '',
              ),
            ],
            ...
            if (valuesDistribution.isNotEmpty)
              ...valuesDistribution.map((v) => _bulletPoint(
                  '${v['value']}: ${v['count']} posts (${v['percent']}%)'))
            else ...[
              _bulletPoint(
                  'Values distribution data will populate from recognition activity'),
            ],
            pw.SizedBox(height: 16),
            _sectionLabel('SECTION 3: COMPLIANCE STATEMENT'),
            pw.SizedBox(height: 8),
            pw.Container(
              ...
              child: pw.Text(
                'This report was automatically generated from real staff activity on the CareKudos platform. '
                'All data is anonymized and compliant with UK GDPR requirements. '
                'Evidence covers staff recognition, values demonstration, and peer interaction patterns.',
    ...
    await _download(
        pdf, 'CareKudos_Compliance_Report_${_ts(now)}.pdf');
  }
  ...
  /// Generate and download a comprehensive Feedback & Recognition Report PDF.
  ///
  /// [users] - user maps: name, role, stars, posts, status
  /// [totalKudos], [totalPosts] - headline metrics
  /// [organizationName] - cover page org name
  /// [managerKudos], [peerKudos], [familyKudos] - recognition by source
  /// [recognitionByCategory] - list of maps: category, count
  /// [topThemes] - list of maps: theme, count, examples
  /// [sentimentPositive], [sentimentNeutral], [sentimentConstructive] - %
  /// [recognitionEquityScore] - 0-100 target 80+
  /// [risingStars] - list of maps: name, team, strength
  /// [managerRecognitionRates] - list of maps: name, percent
  static Future<void> exportRecognitionReport({
      ...
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.SizedBox(height: 20),
            _sectionLabel('RECOGNITION BY SOURCE'),
            pw.SizedBox(height: 14),
            pw.Row(children: [
              _kpiBox('Total Kudos', _fmtNum(totalKudos), 'This period',
                  color: _primary),
              pw.SizedBox(width: 12),
              _kpiBox('Total Posts', _fmtNum(totalPosts), 'Recognition posts',
                  color: _primary),
            ]),
            pw.SizedBox(height: 16),
            if (effectiveTotal > 0) ...[
              ...
              _sourceBar(
                  'Manager Recognition', managerKudos, effectiveTotal, _gold),
              pw.SizedBox(height: 6),
              _sourceBar(
                  'Family Recognition', familyKudos, effectiveTotal, _green),
            ],
            pw.SizedBox(height: 20),
            if (recognitionByCategory.isNotEmpty) ...[
              ...
              ...recognitionByCategory.take(6).map((c) => pw.Padding(
                    ...
                    child: _horizontalBar(
                      ...
                      percent: _safePct(
                          c['count'] as int? ?? 0, effectiveTotal),
                      suffix: '${c['count']}',
                    ),
                  )),
            ],
            ...
            pw.SizedBox(height: 8),
            if (peerKudos > managerKudos)
              _bulletPoint(
                  'Peer recognition dominates - indicates a healthy culture')
            else
              _bulletPoint(
                  'Manager recognition leads - encourage more peer-to-peer recognition'),
            _bulletPoint(
                'Total of ${_fmtNum(totalKudos)} recognition interactions this period'),
            pw.Spacer(),
            _buildPageFooter(ctx),
          ],
        ),
      ...
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.SizedBox(height: 14),
            pw.Row(children: [
              _kpiBox('Positive', '${sentimentPositive.toStringAsFixed(0)}%',
                  'Of all recognition', color: _green),
              pw.SizedBox(width: 10),
              _kpiBox('Neutral', '${sentimentNeutral.toStringAsFixed(0)}%',
                  'Of all recognition', color: _amber),
              pw.SizedBox(width: 10),
              _kpiBox(
                  'Constructive',
                  '${sentimentConstructive.toStringAsFixed(0)}%',
                  'Of all recognition',
                  color: _primary),
            ]),
            pw.SizedBox(height: 24),
            if (topThemes.isNotEmpty) ...[
              ...
              _sectionLabel('RECOGNITION OVERVIEW'),
              pw.SizedBox(height: 10),
              _bulletPoint(
                  'Theme analysis requires recognition post content data'),
              _bulletPoint(
                  'Encourage staff to add detailed descriptions to recognition'),
            ],
            pw.Spacer(),
            _buildPageFooter(ctx),
          ],
    ...
    // ── PAGE 3: TEAM RECOGNITION HEALTH ──
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(40),
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.Row(children: [
              _kpiBox(
                  'Recognition Equity Score',
                  '${recognitionEquityScore.toStringAsFixed(0)}%',
                  recognitionEquityScore >= 80
                      ? 'On target for optimal retention'
                      : 'Target: 80%+ for optimal retention',
                  color: recognitionEquityScore >= 80 ? _green : _amber),
              pw.SizedBox(width: 12),
              _kpiBox('Total Staff', '${users.length}',
                  'In recognition system', color: _primary),
            ]),
            pw.SizedBox(height: 20),
            if (managerRecognitionRates.isNotEmpty) ...[
              _sectionLabel('MANAGER RECOGNITION RATES'),
              pw.SizedBox(height: 10),
              ...managerRecognitionRates.map((m) => pw.Padding(
              ...
              pw.SizedBox(height: 10),
              pw.Text('Target: 80%+ for all managers',
                  style: pw.TextStyle(fontSize: 9, color: _gray)),
            ],
            pw.SizedBox(height: 20),
            _sectionLabel('RECOGNITION GAPS'),
            pw.SizedBox(height: 8),
            // Find least recognized
            _bulletPoint(
                'Most recognized staff (top 10%) receive disproportionate recognition'),
            _bulletPoint(
                'Least recognized staff need targeted manager attention'),
            _bulletPoint(
                '${users.where((u) => (int.tryParse(u['stars'] ?? '0') ?? 0) == 0).length} staff received 0 recognition this period'),
            pw.SizedBox(height: 20),
            _sectionLabel('RECOMMENDATIONS'),
            pw.SizedBox(height: 8),
            _bulletPoint(
                'Spotlight challenge: "Recognize someone new today"'),
            _bulletPoint(
                'Track recognition equity as a KPI for managers'),
            _bulletPoint(
                'Night shift / weekend recognition initiatives needed'),
            _bulletPoint(
                'Coach managers below 80% recognition rate target'),
            pw.Spacer(),
            _buildPageFooter(ctx),
          ],
        ),
    ...
    // ── PAGE 4: RISING STARS & TALENT PIPELINE ──
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(40),
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            if (risingStars.isNotEmpty) ...[
              _sectionLabel(
                  'CURRENT RISING STARS (${risingStars.length} IDENTIFIED)'),
              pw.SizedBox(height: 12),
              pw.TableHelper.fromTextArray(
                headerStyle: pw.TextStyle(
                    fontWeight: pw.FontWeight.bold,
              ...
              pw.SizedBox(height: 12),
              // Derive rising stars from top users by kudos
              ..._deriveRisingStars(users),
            ],
            pw.SizedBox(height: 20),
            _sectionLabel('TALENT RETENTION STRATEGY'),
            pw.SizedBox(height: 8),
            _checkItem(true, 'Identify top performers through recognition data'),
            _checkItem(true, 'Mentorship opportunities from recognition patterns'),
            _checkItem(false, 'Recognition frequency needs improvement for some staff'),
            _checkItem(false, 'Career pathway discussions recommended'),
            pw.SizedBox(height: 20),
            _insightBox(
              'Rising stars identified through CareKudos recognition patterns represent '
              'your strongest retention candidates. Schedule development conversations '
              'to retain and grow this talent pipeline.',
            ),
            pw.Spacer(),
            _buildPageFooter(ctx),
          ],
    ...
    if (users.isNotEmpty) {
      pdf.addPage(
        pw.MultiPage(
          ...
          margin: const pw.EdgeInsets.all(40),
          header: (ctx) =>
              _pageHeaderWidget('RECOGNITION DATA - FULL LISTING'),
          footer: (ctx) => _buildPageFooter(ctx),
          build: (ctx) => [
            pw.SizedBox(height: 12),
            pw.TableHelper.fromTextArray(
    ...
    await _download(
        pdf, 'CareKudos_Recognition_Report_${_ts(now)}.pdf');
  }
        ...
        child: pw.Padding(
          ...
          child: pw.Column(
            ...
            children: [
              ...
              pw.SizedBox(height: 8),
              _coverMeta('Generated:',
                  DateFormat('d MMMM yyyy, HH:mm').format(now)),
              pw.Spacer(),
              // Stars row
              pw.Row(children: List.generate(5, (_) => pw.Padding(
  ...
  static pw.Widget _coverMeta(String label, String value) {
    return pw.Row(children: [
      ...
      pw.Text(value,
          style: pw.TextStyle(
              fontSize: 11, color: _white, fontWeight: pw.FontWeight.bold)),
    ]);
  }
  ...
  static pw.Widget _pageHeaderWidget(String title) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Row(children: [
  ...
  static pw.Widget _complianceGauge(
      String label, double percent, String detail) {
      ...
      child: pw.Container(
        ...
        child: pw.Column(
          children: [
            ...
            pw.SizedBox(height: 8),
            pw.Text('${percent.toStringAsFixed(0)}%',
                style: pw.TextStyle(
                    fontSize: 24,
                    fontWeight: pw.FontWeight.bold,
                    color: color)),
  ...
  static pw.Widget _checkItem(bool done, String text) {
    return pw.Padding(
      padding: const pw.EdgeInsets.only(bottom: 4, left: 8),
      child: pw.Row(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
  ...
  static pw.Widget _alertBox(String text) {
    return pw.Container(
      padding: const pw.EdgeInsets.all(10),
      decoration: pw.BoxDecoration(
        color: const PdfColor.fromInt(0xFFFEF2F2),
        border: pw.Border(
            left: pw.BorderSide(color: _red, width: 3)),
        borderRadius: pw.BorderRadius.circular(4),
      ),
      child: pw.Text(text,
          style: pw.TextStyle(
              fontSize: 9, fontWeight: pw.FontWeight.bold, color: _red)),
    );
  }

  // ─── Quote box for CQC evidence ───

  static pw.Widget _quoteBox({
        ...
        children: [
          ...
          if (value.isNotEmpty || stars.isNotEmpty)
            pw.Padding(
              ...
              child: pw.Text(
                '${stars.isNotEmpty ? "$stars stars - " : ""}demonstrated ${value.toUpperCase()}',
                style: pw.TextStyle(
                    fontSize: 8,
                    fontWeight: pw.FontWeight.bold,
                    color: _gold),
  ...
  static pw.Widget _buildBarChart({
    ...
    required PdfColor color1,
    required PdfColor color2,
    ...
    return pw.Column(
      ...
      children: [
        // Legend
        pw.Row(children: [
          pw.Container(width: 12, height: 8, color: color1),
          pw.SizedBox(width: 4),
          pw.Text(label1,
              style: pw.TextStyle(fontSize: 8, color: _gray)),
          pw.SizedBox(width: 16),
          pw.Container(width: 12, height: 8, color: color2),
          pw.SizedBox(width: 4),
          pw.Text(label2,
              style: pw.TextStyle(fontSize: 8, color: _gray)),
        ]),
                      ...
                      pw.Row(
                        ...
                        children: [
                          pw.Container(
                            width: 10,
                            height: maxVal > 0
                                ? (data1[i] / maxVal * (chartHeight - 20))
                                    .clamp(2.0, chartHeight - 20)
                                : 2,
                            color: color1,
                          ),
                          pw.SizedBox(width: 2),
                          pw.Container(
                            width: 10,
                            height: i < data2.length && maxVal > 0
                                ? (data2[i] / maxVal * (chartHeight - 20))
                                    .clamp(2.0, chartHeight - 20)
                                : 2,
                            color: color2,
                          ),
  ...
  static pw.Widget _horizontalBar({
    ...
    return pw.Row(
      children: [
        ...
        pw.Expanded(
          child: _progressBar((percent / 100).clamp(0.0, 1.0), 10, color),
        ),
        pw.SizedBox(width: 8),
        pw.Text(suffix ?? '${percent.toStringAsFixed(0)}%',
            style: pw.TextStyle(
                fontSize: 9,
                fontWeight: pw.FontWeight.bold,
                color: _darkText)),
  ...
  static pw.Widget _sourceBar(
    ...
    return pw.Row(
      children: [
        ...
        pw.Expanded(
          child: _progressBar((pct / 100).clamp(0.0, 1.0), 10, color),
        ),
        pw.SizedBox(width: 8),
        pw.Text('$count (${pct.toStringAsFixed(0)}%)',
            style: pw.TextStyle(
                fontSize: 9,
                fontWeight: pw.FontWeight.bold,
                color: _darkText)),
  ...
  static pw.Widget _teamCard(Map<String, dynamic> team) {
    ...
    final avgKudos =
        (team['avgKudosPerStaff'] as num?)?.toDouble() ?? 0;
    final topRecognizer = team['topRecognizer'] as String? ?? '';
    final topValue = team['topValue'] as String? ?? '';
    final warnings = team['warnings'] as List<String>? ?? [];

    final pColor = participation >= 80
        ? _green
        : participation >= 50
            ? _amber
            : _red;
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          // Participation bar
          pw.Row(children: [
            pw.Text('Participation: ',
                style: pw.TextStyle(fontSize: 9, color: _gray)),
            pw.Expanded(
              child: _progressBar((participation / 100).clamp(0.0, 1.0), 8, pColor, radius: 4),
            ),
            pw.SizedBox(width: 6),
            pw.Text('${participation.toStringAsFixed(0)}%',
                style: pw.TextStyle(
                    fontSize: 9,
                    fontWeight: pw.FontWeight.bold,
                    color: pColor)),
          ...
          pw.SizedBox(height: 4),
          pw.Text(
              'Avg Kudos/Staff: ${avgKudos.toStringAsFixed(1)}   |   Top Recognizer: $topRecognizer   |   Top Value: $topValue',
              style: pw.TextStyle(fontSize: 8, color: _gray)),
          ...warnings.map((w) => pw.Padding(
                padding: const pw.EdgeInsets.only(top: 4),
                child: pw.Text('! $w',
                    style: pw.TextStyle(
                        fontSize: 8,
  ...
  // ─── Pending training table (derived from users) ───

  static pw.Widget _pendingTrainingTable(List<Map<String, String>> users) {
    final pending = users
        .where((u) =>
            u['gdpr'] == 'Pending' || u['onboarding'] == 'Pending')
        .take(15)
        .toList();
    if (pending.isEmpty) {
      return pw.Text('All staff have completed required training.',
    ...
    return pw.TableHelper.fromTextArray(
      ...
      headerPadding:
          const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 7),
      headers: ['Staff Name', 'Role', 'GDPR', 'Onboarding'],
      data: pending
          .map((u) => [
                u['name'] ?? '',
                u['role'] ?? '',
  ...
  // ─── Derive rising stars from user data ───

  static List<pw.Widget> _deriveRisingStars(List<Map<String, String>> users) {
    final sorted = [...users];
    sorted.sort((a, b) =>
        (int.tryParse(b['stars'] ?? '0') ?? 0)
            .compareTo(int.tryParse(a['stars'] ?? '0') ?? 0));
    final top = sorted.take(7).toList();
    if (top.isEmpty) {
      return [
        pw.Text('No recognition data available yet.',
            style: pw.TextStyle(fontSize: 10, color: _gray))
      ];
    }
    return [
  ...
  // ═══════════════════════════════════════════════════════════════
  // REPORT 4: CQC EVIDENCE REPORT (8 pages) - Web download
  // ═══════════════════════════════════════════════════════════════
  ...
  static Future<void> exportCqcEvidenceReport({
        ...
        children: [
          ...
          pw.Container(
            ...
            child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              _cqcQualityRow('Posts with Reflective Practice:', reflectivePracticePosts, totalRecognitions),
              pw.SizedBox(height: 4),
              _cqcQualityRow('Posts Demonstrating Learning:', learningPosts, totalRecognitions),
              pw.SizedBox(height: 4),
              _cqcQualityRow('Posts Showing Innovation:', innovationPosts, totalRecognitions),
              pw.SizedBox(height: 4),
              _cqcQualityRow('Posts Flagged for GDPR Review:', gdprFlags, totalRecognitions),
            ]),
          ),
          pw.SizedBox(height: 20),
    ...
    pdf.addPage(pw.Page(
      ...
      build: (ctx) => pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(height: 8),
          _checkItem(true, 'All Rising Stars have development plans'),
          _checkItem(true, 'Leadership in Care training enrolled'),
          _checkItem(true, 'Shadowing senior staff programme active'),
          _checkItem(true, 'Mentorship program launched'),
          pw.Spacer(),
          _buildPageFooter(ctx),
        ],
      ),
    ...
    pdf.addPage(pw.Page(
      ...
      build: (ctx) => pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(height: 8),
          _checkItem(true, 'Introduced weekly values focus'),
          _checkItem(true, 'Launched night shift recognition initiative'),
          _checkItem(true, 'Added family feedback portal'),
          _checkItem(true, 'Manager recognition training completed'),
          pw.SizedBox(height: 20),
    ...
    final orgClean = organizationName.replaceAll(RegExp(r'[^a-zA-Z0-9]'), '_');
    final month = DateFormat('MMM_yyyy').format(now);
    await _download(pdf, 'CareKudos_CQC_Evidence_${orgClean}_$month.pdf');
  }

  /// Quality indicator row for CQC report.
  static pw.Widget _cqcQualityRow(String label, int count, int total) {
  ...
  static String _ts(DateTime d) => DateFormat('yyyyMMdd_HHmm').format(d);

  static double _safePct(int count, int total) =>
