...
class ReportService {
  ...
  /// Generate and share/download an Engagement Report PDF.
  static Future<void> exportEngagementReport({
    required List<Map<String, String>> users,
  }) async {
    final pdf = _buildEngagementPdf(users);
    await _sharePdf(pdf, 'engagement_report');
  }

  /// Generate and share/download a Training Compliance Report PDF.
  static Future<void> exportComplianceReport({
    required List<Map<String, String>> users,
    required double gdprPercent,
    required double onboardingPercent,
  }) async {
    final pdf = _buildCompliancePdf(users, gdprPercent, onboardingPercent);
    await _sharePdf(pdf, 'compliance_report');
  }

  /// Generate and share/download a Feedback & Recognition Report PDF.
  static Future<void> exportRecognitionReport({
    required List<Map<String, String>> users,
    required int totalKudos,
    required int totalPosts,
  }) async {
    final pdf = _buildRecognitionPdf(users, totalKudos, totalPosts);
    await _sharePdf(pdf, 'recognition_report');
  }

  /// Generate and share/download a CQC Evidence Report PDF.
  static Future<void> exportCqcReport({
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => _buildHeader('CQC Evidence Report', now),
        footer: (ctx) => _buildFooter(ctx),
        build: (ctx) => [
          pw.SizedBox(height: 20),
          pw.Text('Evidence of Values-Based Culture',
              style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: _navy)),
          pw.SizedBox(height: 16),
          // Summary boxes
          pw.Row(children: [
            _summaryBox('Values Distribution', '$monthlyValuesDistribution values', _navy),
            pw.SizedBox(width: 12),
            _summaryBox('Tagged Recognitions', '$taggedRecognitions', _navy),
            pw.SizedBox(width: 12),
            _summaryBox('Alignment Trend', '+${valuesAlignmentTrend.toStringAsFixed(0)}%', _green),
          ]),
          pw.SizedBox(height: 24),
          pw.Text('Monthly Compliance Overview',
          ...
          pw.Container(
            ...
            child: pw.Text(
              'This report is generated as evidence for CQC inspection. '
              'All data is sourced from the CareKudos platform and reflects '
              'real-time staff recognition activity.',
    ...
    await _sharePdf(pdf, 'cqc_evidence_report');
  }

  // ─── PDF builders ───

  static pw.Document _buildEngagementPdf(List<Map<String, String>> users) {
    final pdf = pw.Document();
    final now = DateTime.now();
    pdf.addPage(
      pw.MultiPage(
  ...
  static pw.Document _buildCompliancePdf(
      List<Map<String, String>> users, double gdprPercent, double onboardingPercent) {
    final pdf = pw.Document();
    final now = DateTime.now();
    pdf.addPage(
      pw.MultiPage(
  ...
  static pw.Document _buildRecognitionPdf(
      List<Map<String, String>> users, int totalKudos, int totalPosts) {
    final pdf = pw.Document();
    final now = DateTime.now();
    pdf.addPage(
      pw.MultiPage(
  ...
  /// Share PDF cross-platform — uses print dialog on mobile, download on web.
